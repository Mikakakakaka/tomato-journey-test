...
(function () {
  ...
  function checkCollision() {
    var px = w * plantX;
    var py = h * 0.65;
    var hitR = Math.min(45, Math.min(w, h) * 0.12);

    waterDrops = waterDrops.filter(function (d) {
      if (Math.hypot(d.x - px, d.y - py) < hitR) {
        growth = Math.min(targetGrowth, growth + 4);
        return false;
      }
      return true;
    });
  ...
  function onTilt(tilt) {
    ...
    var tiltRange = 0.65;
    plantX = Math.max(0.02, Math.min(0.98, (tx + tiltRange) / (tiltRange * 2)));
    plantTilt = tilt.x * 25;
  }
