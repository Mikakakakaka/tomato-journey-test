...
(function (global) {
  ...
  function onKey(e) {
    ...
    if (key === 'ArrowLeft' || key === 'a' || key === 'A') keyTarget.x = -down * keySpeed;
    else if (key === 'ArrowRight' || key === 'd' || key === 'D') keyTarget.x = down * keySpeed;
    else if (key === 'ArrowUp' || key === 'w' || key === 'W') keyTarget.y = -down * keySpeed;
    else if (key === 'ArrowDown' || key === 's' || key === 'S') keyTarget.y = down * keySpeed;
  }
  ...
  function updateKeyTilt() {
    keyTilt.x += (keyTarget.x - keyTilt.x) * 0.22;
    keyTilt.y += (keyTarget.y - keyTilt.y) * 0.22;
  }

  function getTiltWithKeys() {
    updateKeyTilt();
    var t = getTilt();
    var useKeys = (Math.abs(keyTilt.x) > 0.02 || Math.abs(keyTilt.y) > 0.02);
